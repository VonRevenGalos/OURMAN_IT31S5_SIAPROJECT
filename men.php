...
// Add category filter (men's products only - sneakers, running, athletics)
$whereConditions[] = "category IN ('sneakers', 'running', 'athletics')";
...
switch ($sort) {
    ...
    case 'clear':
    default:
        $orderBy = 'ORDER BY id ASC'; // Default order
        break;
}

// Get products - Optimized to select only needed fields
try {
    $sql = "SELECT id, title, price, image, brand, color, stock, category, height, width, collection FROM products $whereClause $orderBy";
...
// Get filter options (only from men's categories) - Optimized with reduced queries
try {
    // Get all unique filter values in fewer queries
    $stmt = $pdo->query("
        SELECT
            color, height, width, brand, collection, category, price
        FROM products
        WHERE category IN ('sneakers', 'running', 'athletics')
    ");
    $allProducts = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Extract unique values from the result set
    $colors = array_unique(array_filter(array_column($allProducts, 'color')));
    $heights = array_unique(array_filter(array_column($allProducts, 'height')));
    $widths = array_unique(array_filter(array_column($allProducts, 'width')));
    $brands = array_unique(array_filter(array_column($allProducts, 'brand')));
    $collections = array_unique(array_filter(array_column($allProducts, 'collection')));
    ...
    $minPrice = !empty($prices) ? min($prices) : 150;
    $maxPrice = !empty($prices) ? max($prices) : 15000;

} catch (PDOException $e) {
    $colors = $heights = $widths = $brands = $collections = $categories = [];
    $minPrice = 150;
    $maxPrice = 15000;
...
<head>
    ...
    <title>Men's Shoes - ShoeARizz</title>
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="page-header">
                        <h1 class="page-title">
                            <i class="fas fa-male me-3"></i>Men's Shoes
                        </h1>
                        <p class="page-subtitle">Discover the latest collection of men's footwear</p>
                    </div>
                </div>
            </div>
                                ...
                                <div class="filter-options">
                                    <?php foreach ($categories as $cat): ?>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="category[]" value="<?php echo htmlspecialchars($cat); ?>" 
                                                   id="category_<?php echo strtolower(str_replace(' ', '_', $cat)); ?>"
                                                   <?php echo (in_array($cat, $category)) ? 'checked' : ''; ?>>
                                            <label class="form-check-label" for="category_<?php echo strtolower(str_replace(' ', '_', $cat)); ?>">
                        ...
                        <div class="filter-group">
                            ...
                            <div class="filter-options">
                                <?php foreach ($categories as $cat): ?>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="category[]" value="<?php echo htmlspecialchars($cat); ?>" 
                                               id="mobile_category_<?php echo strtolower(str_replace(' ', '_', $cat)); ?>"
                                               <?php echo (in_array($cat, $category)) ? 'checked' : ''; ?>>
                                        <label class="form-check-label" for="mobile_category_<?php echo strtolower(str_replace(' ', '_', $cat)); ?>">
                                            <?php echo htmlspecialchars($cat); ?>
                                        </label>
                                    </div>
                                <?php endforeach; ?>
